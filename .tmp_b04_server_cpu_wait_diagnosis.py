...
OUT = ROOT / ".tmp_b04_server_cpu_wait_diagnosis.log"
...
async def seed_tenant() -> tuple[str, str]:
    ...
    try:
        tid = uuid.uuid5(uuid.NAMESPACE_URL, "b04-server-cpu-waits")
        api_key = f"b04_server_{tid}"
        api_hash = hashlib.sha256(api_key.encode()).hexdigest()
        secret = "b04_server_secret"
        await conn.execute(
            """
            INSERT INTO tenants (
            ...
            """,
            str(tid), api_hash, "B04 Server CPU", "b04_server@test.invalid", "s", secret, "p", "w"
        )
...
def _get_worker_processes(master_pid: int) -> list[psutil.Process]:
    try:
    ...
    for p in children:
        try:
            ...
            continue
        if "spawn_main" in cmdline or "uvicorn" in cmdline:
            workers.append(p)
    return workers
...
async def run_with_telemetry(coro, master_pid: int):
    ...
    workers = _get_worker_processes(master_pid)
    for w in workers:
        try:
    ...
    server_cpu_samples: list[float] = []

    async def sample_cpu():
        while not stop["stop"]:
            client_cpu_samples.append(client_proc.cpu_percent(interval=None))
            ws = _get_worker_processes(master_pid)
            total_worker_cpu = 0.0
            for w in ws:
                try:
                    total_worker_cpu += w.cpu_percent(interval=None)
                except Exception:
                    pass
            server_cpu_samples.append(total_worker_cpu)
            await asyncio.sleep(0.2)
    ...
    result["telemetry"] = {
        ...
        "server_workers_cpu_avg_percent": round(sum(server_cpu_samples) / len(server_cpu_samples), 2) if server_cpu_samples else 0.0,
        "server_workers_cpu_peak_percent": round(max(server_cpu_samples), 2) if server_cpu_samples else 0.0,
    }
    return result
...
async def bench_webhook_unique(n: int, conc: int, api_key: str, secret: str):
    ...
    async with httpx.AsyncClient(limits=limits) as c:
        async def one(i: int):
            ...
            async with sem:
                key = str(uuid.uuid5(uuid.NAMESPACE_URL, f"b04-server-key-{i}"))
                body = {
                    "id": f"evt_{i}",
...
def main():
    ...
    cmd = [sys.executable, "-m", "uvicorn", "app.main:app", "--host", "127.0.0.1", "--port", "8060", "--workers", "2", "--no-access-log"]
    ...
    try:
        ...
        res = asyncio.run(run_with_telemetry(bench_webhook_unique(4000, 200, api_key, secret), proc.pid))
        out = {
            "runtime": {
                "uvicorn_workers": 2,
                "database_pool_size": 20,
        ...
        print("B04_SERVER_CPU_WAIT_DIAG_BEGIN")
        print(json.dumps(out, indent=2))
        print("B04_SERVER_CPU_WAIT_DIAG_END")
    finally:
        try:
