"""
VALUE_02 gate runner.

Ensures required schema is migrated, then runs the Value Trace 02 test which
...
def main() -> int:
    ...
    try:
        run(["alembic", "upgrade", "202511131121"], "value_02_alembic_core.log", env=env)
        run(
            ["alembic", "upgrade", "skeldir_foundation@head"],
            "value_02_alembic_foundation.log",
            env=env,
        )
        run(
            [
                ...
                "-m",
                "pytest",
                "backend/tests/value_traces/test_value_02_constraint_trace.py",
                "-q",
            ],
            "value_02_pytest.log",
            env=env,
