"""
VALUE_04 gate runner.

Ensures required schema is migrated, then runs the Value Trace 04 test which
...
def main() -> int:
    ...
    try:
        run(["alembic", "upgrade", "202511131121"], "value_04_alembic_core.log", env=env)
        run(
            ["alembic", "upgrade", "skeldir_foundation@head"],
            "value_04_alembic_foundation.log",
            env=env,
        )
        run(
            [
