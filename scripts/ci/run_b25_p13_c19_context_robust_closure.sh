...
if [[ "$python_bin" == *.exe ]]; then
  ...
  observer_env=(
    ...
    C19_PUBLISHER_DATABASE_URL C19_PUBLISHER_SYNC_DATABASE_URL
    C19_TRANSPORT_DATABASE_URL C19_SIGNER_DATABASE_URL C19_ISSUER_DATABASE_URL
    SKELDIR_B25_P13_C19_TOPOLOGY_PROOF
  ...
  printf '& "%s" -m pytest backend/tests/trust/test_b25_p13_c19_context_robust_topology.py -q -s --no-header -p no:randomly *>&1 | Tee-Object -FilePath "%s"\n' \
