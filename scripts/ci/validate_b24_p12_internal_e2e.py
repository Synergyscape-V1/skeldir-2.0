...
REQUIRED_TEST_CASES = (
    ...
    "test_b24_p12_ca1_subprocess_worker_boundary_consumes_committed_representative_state",
    "test_b24_p12_ca1_subprocess_worker_boundary_negative_control_rejects_uncommitted_state",
    ...
    "test_b24_p12_ca1_runtime_oversized_rejection_occurs_before_graph_or_sampler",
...
def _load_yaml(path: Path) -> Any:
    try:
        ...
        raise ValidationError(
            f"invalid YAML: {path.relative_to(ROOT).as_posix()}: {exc}"
        ) from exc
...
def validate_harness(
    ...
    dispatch_authority_text: str | None = None,
) -> None:
    ...
    dispatch_authority = (
        dispatch_authority_text
        if dispatch_authority_text is not None
        else _read(DISPATCH_AUTHORITY)
    )
    ...
    for token in (
        ...
        "subprocess.Popen",
        "worker_process_id",
        "worker_db_backend_pid",
        "worker_replay_db_backend_pid",
        "P12_CA1_MEMORY_CEILING_BYTES",
        "P12_CA1_WORKER_TELEMETRY_SCHEMA_VERSION",
    ):
    ...
    for token in (
        "P12_CA1_REPRESENTATIVE_EVENT_COUNT",
        "P12_CA1_EXPECTED_SOURCE_ROWS",
        "representative_scale",
        "streamed_source_row_count",
        "evaluate_input_profile_resource_bounds",
        "FallbackReason.INPUT_TOO_LARGE",
    ):
        _require(
            token in tests or token in harness, f"P12 CA1 proof token missing: {token}"
        )
    sentinel = (
    ...
    _require(
        ...
        in claim_wrapper.replace(" ", "").replace("\n", ""),
        "P12 CA1 claim wrapper must bind non-empty RLS sentinel before DB claim",
    )
...
def validate_ci_wiring(
    ...
    for token in (
        ...
        "SKELDIR_B24_P12_CA1_TELEMETRY_DIR",
        "b24-p12-ca1-telemetry",
    ):
        _require(token in workflow, f"P12 workflow wiring missing: {token}")
    _require(
        "b2-4-p12-internal-e2e-proof" in workflow
        and "b2-4-p12-internal-e2e-proof"
        in workflow.split("b2-4-p11-ci-gates-and-negative-control-harness", 1)[1],
        "P11 job must need P12 before artifact adjudication",
    )
    _require(P12_MAKE_TARGET in makefile, "P12 Makefile target missing")
    _require(
        P12_VALIDATOR_COMMAND in makefile, "P12 Makefile validator command missing"
    )
    _require(
        "id: validate-b24-p12-internal-e2e" in registry, "P12 registry entry missing"
    )
    ...
    _require(
        P12_NON_OVERCLAIM in evidence,
        "P12 evidence pack missing non-overclaim boundary",
    )
    for token in (
        "transaction visibility proof",
        "uncommitted visibility negative control",
        "worker/subprocess boundary proof",
        "representative resource telemetry proof",
...
def validate_manifest(manifest_text: str | None = None) -> None:
    ...
    p12_rows = [
        row
        for row in rows
        if isinstance(row, dict) and row.get("phase_id") == "B2.4-P12"
    ]
    _require(len(p12_rows) == 1, "P12 execution manifest row must exist exactly once")
    row = p12_rows[0]
    _require(
        row.get("workflow_job") == P12_CONTEXT, "P12 manifest workflow job mismatch"
    )
    _require(
        row.get("required_status") == P12_CONTEXT,
        "P12 manifest required status mismatch",
    )
    _require(
        row.get("execution_required") is True, "P12 manifest must require execution"
    )
    ...
    fragments = "\n".join(
        str(item) for item in row.get("required_command_fragments", [])
    )
    _require(
        "pytest backend/tests/test_b24_p12_internal_e2e.py" in fragments,
        "P12 manifest missing pytest command",
    )
    _require(
        "validate_b24_p12_internal_e2e.py --negative-control" in fragments,
        "P12 manifest missing validator command",
    )
...
def run_negative_controls() -> None:
    ...
    _expect_failure(
        "ca1_worker_boundary_case_removed",
        lambda: validate_harness(
            tests_text=tests.replace(
                "test_b24_p12_ca1_subprocess_worker_boundary_consumes_committed_representative_state",
                "removed",
    ...
    _expect_failure(
        "ca1_subprocess_removed",
        lambda: validate_harness(
            harness_text=harness.replace("subprocess.Popen", "subprocess_removed")
        ),
        "subprocess",
    )
    _expect_failure(
        "ca1_oversized_runtime_rejection_removed",
        lambda: validate_harness(
            tests_text=tests.replace(
                "FallbackReason.INPUT_TOO_LARGE", "FallbackReason.REMOVED"
            )
        ),
        "ca1",
    )
    ...
    _expect_failure(
        "monotonic_removed",
        lambda: validate_harness(
            harness_text=harness.replace("time.monotonic()", "time.time()")
        ),
        "monotonic",
    )
    _expect_failure(
        "prompt_control_validator_removed",
        lambda: validate_projection_schema(
            metadata_text=metadata.replace(
                "_PROMPT_CONTROL_TOKENS", "PROMPT_CONTROL_REMOVED"
            )
        ),
        "prompt",
    )
    _expect_failure(
        ...
        lambda: validate_ci_wiring(
            workflow_text=workflow.replace(P12_CONTEXT, "B2.4-P12 Missing")
        ),
        "workflow",
    )
    _expect_failure(
        "ca1_telemetry_artifact_removed",
        lambda: validate_ci_wiring(
            workflow_text=workflow.replace(
                "b24-p12-ca1-telemetry", "b24-p12-ca1-removed"
            )
        ),
        "telemetry",
    )
    _expect_failure(
        "make_target_removed",
        lambda: validate_ci_wiring(
            makefile_text=makefile.replace(P12_MAKE_TARGET, "validate-b24-p12-removed")
        ),
        "Makefile",
    )
    _expect_failure(
        ...
        lambda: validate_ci_wiring(
            registry_text=registry.replace(
                "id: validate-b24-p12-internal-e2e", "id: validate-b24-p12-removed"
            )
        ),
        "registry",
    )
    ...
    _expect_failure(
        ...
        lambda: validate_ci_wiring(
            evidence_text=evidence.replace(
                P12_NON_OVERCLAIM, "P12 proves production topology trust closure."
            )
        ),
        "non-overclaim",
    )
    ...
    _expect_failure(
        "subsumption_removed",
        lambda: validate_subsumption(
            subsumption_text=subsumption.replace(
                "gate_id: validate-b24-p12-internal-e2e", "gate_id: removed"
            )
        ),
        "subsumption",
    )
