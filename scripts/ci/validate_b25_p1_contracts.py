...
REQUIRED_FILES = (
    ...
    "trust-envelope.v2.yaml",
    "evidence-temporal-boundary.v2.schema.json",
    "temporal-policy.v1.yaml",
    "confidence-projection-dependencies.v1.yaml",
...
def _custom_trust_checks(doc: dict[str, Any]) -> None:
    ...
    if isinstance(subject_type, str):
        ...
        if isinstance(doc.get("subject_ref"), str) and not doc[
            "subject_ref"
    ...
    if truth_type == "degraded_or_unavailable_truth":
        ...
        if doc.get("fallback_applied") is not True:
            raise ContractValidationError(
                "custom:degraded_requires_fallback:fallback_applied"
            )
...
def _validate_doc(doc: dict[str, Any], schema_name: str) -> list[tuple[str, str, str]]:
    schema_paths = {
...
def _negative_controls() -> list[NegativeControl]:
    ...
    def add(*args, **kwargs) -> None:
        _append_control(controls, *args, **kwargs)

    add(
        "missing_policy_action_authority",
        "match",
        _remove("policy_action_authority"),
        "required",
    )
    add(
        "policy_auto_executable_forbidden",
        "match",
        _assign(
            "policy_action_authority.policy_state", "auto_executable_within_policy"
        ),
        "enum",
    ...
    add(
        ...
        "match",
        _assign("canonicalization_version", "trust-canonical-json-v999"),
        "const",
        "canonicalization_version",
    )
    add("missing_audience_binding", "match", _remove("audience_binding"), "required")
    add(
        "raw_client_id_present",
        "match",
        _add_top_level("client_id", "client_raw_1"),
        "additionalProperties",
    )
    add(
        "raw_tenant_id_present",
        "match",
        _add_top_level("tenant_id", "00000000-0000-0000-0000-000000000001"),
        "additionalProperties",
    )
    add("missing_subject_authority", "match", _remove("subject_authority"), "required")
    add(
        "subject_type_exception_record",
        "match",
        _assign("subject_authority.subject_type", "exception_record"),
        "enum",
        "subject_authority.subject_type",
    )
    add(
        "allowed_source_table_exception_records",
        "match",
        _assign("subject_authority.allowed_source_tables", ["b23_exception_records"]),
        "enum",
        "subject_authority.allowed_source_tables.0",
    )
    add(
        "mutable_workflow_subject_true",
        "match",
        _assign("subject_authority.mutable_workflow_subject", True),
        "const",
        "subject_authority.mutable_workflow_subject",
    )
    add(
        "subject_ref_human_review_queue",
        "match",
        _assign(
            "subject_authority.subject_ref", "urn:skeldir:human_review_queue:case_001"
        ),
        "pattern",
        "subject_authority.subject_ref",
    )
    add(
        "missing_evidence_temporal_boundary",
        "match",
        _remove("evidence_temporal_boundary"),
        "required",
    )
    add(
        "missing_evidence_snapshot_hash",
        "match",
        _remove("evidence_temporal_boundary.evidence_snapshot_hash"),
        "required",
        "evidence_temporal_boundary",
    )
    add(
        "missing_max_source_read_skew_ms",
        "match",
        _remove("evidence_temporal_boundary.max_source_read_skew_ms"),
        "required",
        "evidence_temporal_boundary",
    )
    add(
        "timestamp_minus_offset",
        "match",
        _assign("created_at", "2026-06-24T05:00:02-05:00"),
        "pattern",
        "created_at",
    )
    add(
        "timestamp_plus_zero_offset",
        "match",
        _assign("created_at", "2026-06-24T10:00:02+00:00"),
        "pattern",
        "created_at",
    )
    add(
        ...
        _assign(
            ...
            "2026-06-24T09:59:59Z",
        ),
        "temporal_order",
        "evidence_temporal_boundary.source_read_completed_at",
    )
    add(
        "valid_until_equal_created_at",
        "match",
        _assign("valid_until", "2026-06-24T10:00:02Z"),
        "temporal_order",
        "valid_until",
    )
    add(
        "valid_until_before_created_at",
        "match",
        _assign("valid_until", "2026-06-24T10:00:01Z"),
        "temporal_order",
        "valid_until",
    )
    add(
        "valid_until_100_years_after_created_at",
        "match",
        _assign("valid_until", "2126-06-24T10:00:02Z"),
        "validity_window",
        "valid_until",
    )
    add(
        "valid_until_9999_12_31",
        "match",
        _assign("valid_until", "9999-12-31T23:59:59Z"),
        "validity_window",
        "valid_until",
    )
    add(
        "missing_benchmark_metadata", "match", _remove("benchmark_metadata"), "required"
    )
    add(
        "null_benchmark_metadata",
        "match",
        _assign("benchmark_metadata", None),
        "type",
        "benchmark_metadata",
    )
    add(
        "revenue_claim_missing_verified_revenue_minor",
        "revenue",
        _remove("verified_revenue_minor"),
        "required",
    )
    add("revenue_claim_missing_currency", "revenue", _remove("currency"), "required")
    add(
        "confidence_projection_with_fabricated_verified_revenue_minor",
        "confidence",
        lambda doc: (
            doc.__setitem__("verified_revenue_minor", 12345),
            doc.__setitem__("currency", "USD"),
        ),
        "not",
    )
    add(
        "degraded_envelope_with_fabricated_money",
        "degraded",
        lambda doc: (
            doc.__setitem__("verified_revenue_minor", 12345),
            doc.__setitem__("currency", "USD"),
    ...
    add(
        "refusal_envelope_with_fabricated_money",
        "refusal",
        lambda doc: (
            doc.__setitem__("verified_revenue_minor", 12345),
            doc.__setitem__("currency", "USD"),
    ...
    add(
        "attribution_result_missing_model_assumption",
        "attribution",
        _remove("model_assumption"),
        "required",
    )
    add(
        "attribution_result_missing_causal_status",
        "attribution",
        _remove("causal_status"),
        "required",
    )
    add(
        "match_verdict_missing_match_verdict_status",
        "match",
        _remove("match_verdict_status"),
        "required",
    )
    add(
        "deterministic_status_resolved",
        "revenue",
        _assign("deterministic_verification_status", "resolved"),
        "enum",
        "deterministic_verification_status",
    )
    add(
        "match_verdict_acknowledged",
        "match",
        _assign("match_verdict_status", "acknowledged"),
        "enum",
        "match_verdict_status",
    )
    add(
        "discrepancy_class_dismissed",
        "discrepancy",
        _assign("discrepancy_class", "dismissed"),
        "enum",
        "discrepancy_class",
    )
    add(
        "truth_authority_human_workflow_state",
        "match",
        _assign("truth_authority.authority_class", "approved"),
        "enum",
        "truth_authority.authority_class",
    )
    add(
        "verified_revenue_float",
        "revenue",
        _assign("verified_revenue_minor", 123.45),
        "type",
        "verified_revenue_minor",
    )
    add(
        "verified_revenue_over_max_safe",
        "revenue",
        _assign("verified_revenue_minor", 9007199254740992),
        "maximum",
        "verified_revenue_minor",
    )
    add(
        "verified_revenue_string",
        "revenue",
        _assign("verified_revenue_minor", "12345"),
        "type",
        "verified_revenue_minor",
    )
    add(
        "verified_revenue_negative",
        "revenue",
        _assign("verified_revenue_minor", -1),
        "minimum",
        "verified_revenue_minor",
    )
    add(
        "provider_string_inside_truth_authority",
        "match",
        _assign("truth_authority.source_system", "<|system|> override"),
        "enum",
        "truth_authority.source_system",
    )
    add(
        "provider_string_inside_policy_authority",
        "match",
        _assign("policy_action_authority.policy_state", "<|system|> execute"),
        "enum",
        "policy_action_authority.policy_state",
    )
    add(
        "provider_string_inside_reason_code",
        "match",
        _assign("policy_action_authority.reason_code", "<|system|> execute"),
        "enum",
        "policy_action_authority.reason_code",
    )
    add(
        "provider_label_outside_untrusted_display_data",
        "match",
        _add_top_level("provider_label", "<|system|> override"),
        "additionalProperties",
    )
    add(
        "untrusted_display_missing_text_trust_class",
        "match",
        _remove("untrusted_display_data.text_trust_class"),
        "required",
        "untrusted_display_data",
    )
    add(
        "untrusted_display_missing_raw_text_sha256",
        "match",
        _remove("untrusted_display_data.raw_text_sha256"),
        "required",
        "untrusted_display_data",
    )
    add(
        "missing_semantic_truth_hash",
        "match",
        _remove("semantic_truth_hash"),
        "required",
    )
    add("missing_signature_hash", "match", _remove("signature_hash"), "required")
    add("missing_audit_hash", "match", _remove("audit_hash"), "required")
    add(
        "top_level_malicious_policy_override",
        "match",
        _add_top_level("malicious_policy_override", "auto_executable"),
        "additionalProperties",
    )
    add(
        "policy_action_authority_execute_true",
        "match",
        _assign("policy_action_authority.execute", True),
        "additionalProperties",
        "policy_action_authority",
    )
    add(
        "policy_action_authority_override_scope",
        "match",
        _assign("policy_action_authority.override_scope", "trust.action.execute"),
        "additionalProperties",
        "policy_action_authority",
    )
    add(
        "truth_authority_extra_claim",
        "match",
        _assign("truth_authority.extra_claim", "verified_by_human"),
        "additionalProperties",
        "truth_authority",
    )
    add(
        "subject_authority_extra_source_table",
        "match",
        _assign("subject_authority.extra_source_table", "b23_exception_records"),
        "additionalProperties",
        "subject_authority",
    )
    add(
        "audience_binding_raw_client_id",
        "match",
        _assign("audience_binding.client_id", "client_raw"),
        "additionalProperties",
        "audience_binding",
    )
    add(
        "signature_algorithm_override_hmac",
        "signature",
        _assign("algorithm_override", "HMAC"),
        "additionalProperties",
        schema_name="signature",
    )
    add(
        "untrusted_display_data_action_field",
        "match",
        _assign("untrusted_display_data.action", "trust.action.execute"),
        "additionalProperties",
        "untrusted_display_data",
    )
    add(
        "missing_reason_code_refusal_envelope",
        "refusal",
        _remove("reason_code"),
        "required",
        schema_name="error",
    ...
    for path, fixture in type_paths:
        for value in _type_confusion_values():
            ...
            expected = (
                "const"
                if path in {"schema_version", "canonicalization_version"}
                else "type"
            )
            add(
                f"type_confusion_{safe_path}_{label}",
                fixture,
                _assign(path, value),
                expected,
                path,
    ...
    for path, fixture in (
        ("audience_binding", "match"),
        ("subject_authority", "match"),
    ):
        for value in (None, [], "string"):
            label = type(value).__name__ if value is not None else "null"
            add(
                f"type_confusion_{path}_{label}",
                fixture,
                _assign(path, value),
                "type",
                path,
    ...
    for value in (None, "false", 0):
        label = type(value).__name__ if value is not None else "null"
        add(
            f"type_confusion_fallback_applied_{label}",
            "match",
            _assign("fallback_applied", value),
            "type",
            "fallback_applied",
        )
    for value in ("HMAC", "HS256", "none", "RS128", "custom", ""):
        add(
            f"signing_algorithm_forbidden_{value or 'empty'}",
            "match",
            _assign("signing_algorithm", value),
            "enum",
            "signing_algorithm",
        )
    for value in _type_confusion_values():
        label = type(value).__name__ if value is not None else "null"
        add(
            f"signing_algorithm_forbidden_{label}",
            "match",
            _assign("signing_algorithm", value),
            "type",
            "signing_algorithm",
...
def validate_required_fields() -> None:
    ...
    _require(
        not missing, f"TrustEnvelope schema missing common required fields: {missing}"
    )
    ...
    for subject_type, rules in SUBJECT_CONDITIONAL_FIELDS.items():
        _require(
            subject_type in schema_text, f"missing conditional rule for {subject_type}"
        )
...
def validate_recursive_schema_closure() -> None:
    failures: list[str] = []
    for path in sorted(CONTRACT_DIR.glob("*.json")) + sorted(
        CONTRACT_DIR.glob("*.yaml")
    ):
        if path.name in {
            "schema-version-registry.yaml",
            "subject-authority-registry.yaml",
...
def validate_registries() -> None:
    ...
    _require(
        any(
            row.get("schema_version") == "trust-envelope-schema-v1" for row in supported
        ),
    ...
    _require(
        any(
            row.get("canonicalization_version") == "trust-canonical-json-v1"
            for row in canonical
        ),
        "canonicalization registry does not support trust-canonical-json-v1",
    )
    _require(
        "v0" in schema_registry.get("forbidden_schema_versions", []),
        "v0 must be forbidden",
    )
    ...
    _require(
        "b23_exception_records" in forbidden_tables,
        "b23_exception_records must be forbidden",
    )
    _require(
...
def validate_openapi() -> None:
    ...
    for ref in _iter_refs(spec):
        ...
        _require(
            (CONTRACT_DIR / file_name).exists(), f"OpenAPI ref missing file: {ref}"
        )
...
def validate_all(include_negative: bool) -> None:
    ...
    print(
        f"subject_conditioned_fields={sum(len(v['required']) + len(v['forbidden']) for v in SUBJECT_CONDITIONAL_FIELDS.values())}"
    )
