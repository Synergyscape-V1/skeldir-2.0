...
def test_b26_p1_migration_graph_is_alembic_native() -> None:
    ...
    # Corrective V advances the single head 202609180001 -> 202609190001
    # (one execution tuple, consequence-bound conducted, staleness
    # signal, least-privilege recovery); ancestry remains linear.
    assert set(graph) == native_revisions
    assert "202512151410" in graph
    assert "202511171000" not in graph
    assert heads == set(script.get_heads()) == {"202609190001"}
