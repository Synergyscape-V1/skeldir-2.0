...
Test Scenario:
1. Create job at t=0 -> expect SUBMITTED
2. Poll at t=+30s -> must still be SUBMITTED (fail if READY/COMPLETED)
3. Poll at t=+46s -> expect READY_FOR_REVIEW
4. Try to fetch "final" before approve -> must not be COMPLETED
5. Approve -> expect APPROVED
6. Complete -> expect COMPLETED
...
async def test_value_trace_centaur_friction_enforced():
    ...
    # Step 1: Create job at t=0 -> expect SUBMITTED
    ...
    assert job.status == InvestigationStatus.SUBMITTED, (
        f"Job should be SUBMITTED at creation, got {job.status}"
    )
    ...
    transitions.append(
        {
            ...
            "assertion": "SUBMITTED with 45s hold",
        }
    )

    # Step 2: Poll at t=+30s -> must still be SUBMITTED
    clock.advance(30)
    ...
    assert job_30s.status == InvestigationStatus.SUBMITTED, (
        f"Job must still be SUBMITTED at t=30s (before min_hold), got {job_30s.status}"
    )
    ...
    transitions.append(
        {
            ...
            "assertion": "Still SUBMITTED (min_hold not reached)",
        }
    )
    ...
    assert job_46s.ready_for_review_at is not None, "ready_for_review_at should be set"
    ...
    assert job_46s.approved_at is None, "approved_at must be None before approval"
    assert job_46s.completed_at is None, "completed_at must be None before completion"
    ...
    # Step 5: Approve -> expect APPROVED
    async with engine.begin() as conn:
        await conn.execute(
            text("SELECT set_config('app.current_tenant_id', :tid, false)"),
            {"tid": str(tenant_id)},
    ...
    assert job_approved.status == InvestigationStatus.APPROVED, (
        f"Job should be APPROVED after approval, got {job_approved.status}"
    )
    assert job_approved.approved_at is not None, "approved_at must be set after approval"
    assert job_approved.completed_at is None, "completed_at must be unset after approval"
    ...
    transitions.append(
        {
            ...
            "approved_at": job_approved.approved_at.isoformat()
            if job_approved.approved_at
            else None,
            "completed_at": None,
            "assertion": "APPROVED (human review accepted)",
        }
    )

    # Step 6: Complete -> expect COMPLETED
    async with engine.begin() as conn:
        await conn.execute(
            text("SELECT set_config('app.current_tenant_id', :tid, false)"),
            {"tid": str(tenant_id)},
    ...
    async with engine.begin() as conn:
        ...
        job_completed = await service.get_job(conn, tenant_id, job.id)

    assert job_completed is not None
    assert job_completed.status == InvestigationStatus.COMPLETED, (
        f"Job should be COMPLETED after explicit completion, got {job_completed.status}"
    )
    assert job_completed.completed_at is not None, "completed_at must be set after completion"

    transitions.append(
        {
            "time": "t=46s (after completion)",
            "action": "complete_job",
            "status": job_completed.status.value,
            "approved_at": job_completed.approved_at.isoformat()
            if job_completed.approved_at
            else None,
            "completed_at": job_completed.completed_at.isoformat()
            if job_completed.completed_at
            else None,
            "assertion": "COMPLETED (service-mediated post-approval terminalization)",
        }
    )
    ...
    with EVIDENCE_MD.open("w", encoding="utf-8") as fh:
        ...
        fh.write("1. Create job at t=0 -> SUBMITTED with 45s hold\n")
        fh.write("2. Poll at t=30s -> still SUBMITTED (15s remaining)\n")
        fh.write("3. Poll at t=46s -> READY_FOR_REVIEW (hold passed)\n")
        fh.write("4. Check before approve -> NOT COMPLETED\n")
        fh.write("5. Approve -> APPROVED\n")
        fh.write("6. Complete -> COMPLETED\n\n")
        fh.write("## State Transitions\n\n")
        fh.write("| Time | Action | Status | Assertion |\n")
        fh.write("|------|--------|--------|----------|\n")
        for transition in transitions:
            fh.write(
                f"| {transition['time']} | {transition['action']} | {transition['status']} | {transition['assertion']} |\n"
            )
        fh.write("\n## SQL Proof Query\n\n")
        fh.write("```sql\n")
        fh.write(sql_proof)
        ...
        fh.write("- [x] State machine integrity (SUBMITTED -> READY -> APPROVED -> COMPLETED)\n")
        fh.write("- [x] Cannot return 'final' immediately\n")


@pytest.mark.asyncio
async def test_cannot_approve_submitted_job():
    """Verify that approving a SUBMITTED job fails."""
    tenant_record = await build_tenant(name="ValueTrace Centaur Submitted Tenant")
    tenant_id = tenant_record["tenant_id"]
    ...
    async with engine.begin() as conn:
        ...
        with pytest.raises(ValueError, match="Illegal investigation transition"):
