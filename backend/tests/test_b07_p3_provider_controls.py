...
from app.models.llm import LLMBudgetReservation, LLMHourlyShutoffState, LLMApiCall, LLMMonthlyBudgetState
...
async def test_p3_cache_hit_and_watermark_invalidation(test_tenant):
    base_prompt = {"simulated_output_text": "cache-me", "cache_enabled": True, "cache_watermark": 7}
    ...
    async with get_session(tenant_id=test_tenant, user_id=SYSTEM_USER_ID) as session:
        first = await generate_explanation(_payload(test_tenant, request_id=first_id, prompt=base_prompt), session=session)
        second = await generate_explanation(_payload(test_tenant, request_id=second_id, prompt=base_prompt), session=session)
        invalidating_prompt = {**base_prompt, "cache_watermark": 8}
        third = await generate_explanation(_payload(test_tenant, request_id=third_id, prompt=invalidating_prompt), session=session)
...
async def test_p3_reservation_concurrency_safety(monkeypatch, test_tenant):
    ...
    async def _invoke(i: int):
        ...
        prompt = {"simulated_delay_ms": 250, "simulated_cost_cents": 1, "cache_enabled": False}
        async with get_session(tenant_id=test_tenant, user_id=SYSTEM_USER_ID) as session:
    ...
    async with get_session(tenant_id=test_tenant, user_id=SYSTEM_USER_ID) as session:
        month = date(datetime.now(timezone.utc).year, datetime.now(timezone.utc).month, 1)
        budget = (
...
async def test_p3_hourly_shutoff_distinct_from_monthly(monkeypatch, test_tenant):
    ...
    async with get_session(tenant_id=test_tenant, user_id=SYSTEM_USER_ID) as session:
        one = await generate_explanation(_payload(test_tenant, request_id=str(uuid4()), prompt=prompt), session=session)
        two = await generate_explanation(_payload(test_tenant, request_id=str(uuid4()), prompt=prompt), session=session)
        three = await generate_explanation(_payload(test_tenant, request_id=str(uuid4()), prompt=prompt), session=session)

    assert one["status"] == "accepted"
    assert two["status"] == "accepted"
    assert three["status"] == "blocked"
    assert three["blocked_reason"] in {"hourly_threshold_exceeded", "hourly_shutoff_active"}
...
async def test_p3_breaker_opens_after_three_failures(monkeypatch, test_tenant):
    ...
    async with get_session(tenant_id=test_tenant, user_id=SYSTEM_USER_ID) as session:
        for _ in range(3):
            res = await generate_explanation(_payload(test_tenant, request_id=str(uuid4()), prompt=fail_prompt), session=session)
            assert res["status"] == "failed"
        blocked = await generate_explanation(_payload(test_tenant, request_id=str(uuid4()), prompt=ok_prompt), session=session)
...
async def test_p3_reservation_and_settlement_use_separate_transactions(monkeypatch, test_tenant):
    ...
    async with get_session(tenant_id=test_tenant, user_id=SYSTEM_USER_ID) as session:
        ...
        monkeypatch.setattr(_PROVIDER_BOUNDARY, "_provider_call", _inspect_provider, raising=True)
...
async def test_p3_distillation_capture_default_false(test_tenant):
    ...
    async with get_session(tenant_id=test_tenant, user_id=SYSTEM_USER_ID) as session:
        result = await generate_explanation(
            _payload(test_tenant, request_id=request_id, prompt={"simulated_output_text": "trace-me"}),
            session=session,
        )
...
async def test_p3_secret_not_leaked_on_failure(monkeypatch, test_tenant):
    ...
    async with get_session(tenant_id=test_tenant, user_id=SYSTEM_USER_ID) as session:
        result = await generate_explanation(
            _payload(test_tenant, request_id=request_id, prompt={"cache_enabled": False}),
            session=session,
        )
...
@pytest.mark.asyncio
async def test_p3_provider_enabled_routes_through_aisuite_boundary(monkeypatch, test_tenant):
    monkeypatch.setattr(settings, "LLM_PROVIDER_ENABLED", True, raising=False)
