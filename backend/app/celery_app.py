...
def _ensure_celery_configured():
    ...
    celery_app.conf.update(
        ...
        task_queues=[
            ...
            Queue(QUEUE_B26_P2_RELAY, routing_key=f'{QUEUE_B26_P2_RELAY}.#'),
        ],
        task_routes={
            ...
            'app.tasks.b26_p2_relay.*': {'queue': QUEUE_B26_P2_RELAY, 'routing_key': f'{QUEUE_B26_P2_RELAY}.task'},
...
def _on_task_failure(task_id=None, exception=None, args=None, kwargs=None, einfo=None, **extra):
    ...
    try:
        ...
        try:
            ...
            sanitized_args = sanitize_output_payload(
                sanitize_for_transport(serialized_args)
            )
            sanitized_kwargs = sanitize_output_payload(
                sanitize_for_transport(serialized_kwargs)
            )
            sanitized_error_message = (
                redact_output_text(redact_text_fragments(str(exception)[:500]))
                if exception
                else ""
            )
            sanitized_traceback = (
                redact_output_text(redact_text_fragments(str(einfo)[:2000]))
                if einfo
                else None
            )
