...
def assert_worker_auth_envelope_active(
    *,
    auth_token: str,
    tenant_id: UUID,
    user_id: UUID,
    ...
    """
    Enforce access-token revocation for worker tasks carrying auth envelopes.
    """
    claims = decode_worker_auth_token(auth_token)
    token_claims = extract_access_token_claims(claims)
    if token_claims.tenant_id != tenant_id:
        raise ValueError("auth envelope tenant mismatch")
    if token_claims.user_id != user_id:
        raise ValueError("auth envelope user mismatch")
