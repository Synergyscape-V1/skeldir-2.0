...
async def wait_ready(timeout_s: int = 90):
    async with httpx.AsyncClient() as c:
        ...
        while time.perf_counter()-start < timeout_s:
            try:
                r=await c.get(BASE+PATH, timeout=3)
    ...
    async with httpx.AsyncClient(limits=limits) as c:
        async def one(i:int):
            ...
            async with sem:
                ...
                t=time.perf_counter()
                try:
                    r=await c.get(BASE+PATH, headers=headers, timeout=30)
...
def main():
    ...
    cmd=[sys.executable,"-m","uvicorn","app.main:app","--host","127.0.0.1","--port","8031","--workers","2","--no-access-log"]
    ...
    try:
        ...
        ka=asyncio.run(run_case("keepalive", n=1000, conc=80, keepalive=80))
        cl=asyncio.run(run_case("connection_close", n=1000, conc=80, keepalive=0))
        out={
            "keepalive":ka,
            "connection_close":cl,
            "delta":{
        ...
        print("CONNECTION_COST_READY_AB_BEGIN")
        print(json.dumps(out, indent=2))
        print("CONNECTION_COST_READY_AB_END")
    finally:
        try:
