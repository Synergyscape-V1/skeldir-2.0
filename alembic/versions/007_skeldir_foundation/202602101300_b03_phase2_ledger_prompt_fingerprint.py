...
def downgrade() -> None:
    ...
    op.execute("ALTER TABLE llm_call_audit DROP COLUMN IF EXISTS prompt_fingerprint")

    op.execute("DROP INDEX IF EXISTS idx_llm_api_calls_prompt_fingerprint")
    op.execute("ALTER TABLE llm_api_calls DROP COLUMN IF EXISTS prompt_fingerprint")
