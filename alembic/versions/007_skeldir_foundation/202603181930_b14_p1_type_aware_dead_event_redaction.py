"""B1.4-P1 corrective: enforce type-aware dead-event redaction masks.

Revision ID: 202603181930
Revises: d92ec17165fe
Create Date: 2026-03-18 19:30:00
"""
...
revision: str = "202603181930"
down_revision: Union[str, None] = "d92ec17165fe"
...
def _type_aware_unredacted_detection_cases(field_name: str) -> str:
    return "\n".join(
        [
            "            IF EXISTS ("
            " SELECT 1"
            f" FROM jsonb_path_query({field_name}, '$.**.{key}') AS pii(value)"
            " WHERE pii.value <> to_jsonb('[REDACTED]'::text)"
            "   AND pii.value <> to_jsonb('[REDACTED_B1.4]'::text)"
            "   AND pii.value <> '0'::jsonb"
            "   AND pii.value <> 'false'::jsonb"
            "   AND pii.value <> '[]'::jsonb"
...
def _legacy_unredacted_detection_cases(field_name: str) -> str:
    return "\n".join(
        [
...
def upgrade() -> None:
    ...
    unredacted_cases_dead = _type_aware_unredacted_detection_cases("NEW.raw_payload")
...
def downgrade() -> None:
    strict_cases_events = _strict_detection_cases("NEW.raw_payload")
    strict_cases_metadata = _strict_detection_cases("NEW.metadata")
    legacy_dead_cases = _legacy_unredacted_detection_cases("NEW.raw_payload")
