/**
 * A2-MERIDIAN ChannelPerformanceTable — Executive Channel View
 */
...
import { type ChannelPerformance, type ConfidenceTier, formatConfidenceLabel, CHANNEL_COLORS } from '../types';

interface Props { channels: ChannelPerformance[]; loading?: boolean; }
    ...
    <Card className="p-6">
      ...
      <CardContent className="p-0">
        <div className="border border-border rounded-md overflow-hidden" role="table" aria-label="Channel performance">
          <div className="grid grid-cols-[1fr_90px_100px_70px_160px_40px] gap-2 px-4 py-2.5 bg-muted/30 border-b border-border" role="row">
            {headers.map(h => (
              <div key={h.field} className={cn('flex items-center gap-1', h.align)} role="columnheader">
                <Button variant="ghost" size="sm" className="h-auto p-0 text-[11px] font-medium text-muted-foreground uppercase tracking-wider hover:text-foreground" onClick={() => toggle(h.field)}>
                  {h.label}
                  {sortField === h.field && <ArrowUpDown className="h-3 w-3 ml-1 text-primary" />}
                </Button>
              </div>
            ))}
            <div role="columnheader" className="text-[11px] font-medium text-muted-foreground uppercase text-center">Trend</div>
          </div>
          ...
          {sorted.map(ch => {
            ...
            return (
              <div key={ch.channelId} className="grid grid-cols-[1fr_90px_100px_70px_160px_40px] gap-2 px-4 py-3 border-b border-border last:border-b-0 hover:bg-accent/50 transition-colors duration-200" role="row">
                ...
                <div className="font-mono text-sm text-foreground text-right tabular-nums" role="cell">${ch.spend.toLocaleString()}</div>
                <div className="font-mono text-sm text-foreground text-right tabular-nums" role="cell">${ch.revenue.toLocaleString()}</div>
                <div className="font-mono text-sm text-foreground text-right tabular-nums" role="cell">{ch.roas.toFixed(2)}</div>
                <div role="cell">
