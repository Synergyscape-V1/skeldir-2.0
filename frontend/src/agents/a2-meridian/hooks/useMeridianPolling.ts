/**
 * A2-MERIDIAN Polling Hook
 *
 * Wraps the shared usePollingManager for Meridian's executive layout.
 ...
 * - Provides isUpdating flag for triggering BreathingHorizonGlow pulse
 *
 * Architecture compliance:
 *   - All data fetching in hooks, never in components
 *   - No UI business logic — returns backend-shaped data as-is
 *   - 1200ms loading → ready transition on mount
 */
...
interface MeridianPollingResult {
...
export function useMeridianPolling(): MeridianPollingResult {
  ...
  const handleDashboardPoll = useCallback(async () => {
    // Signal update pulse for BreathingHorizonGlow
    setIsUpdating(true);
  ...
  // Suppress unused variable warning — channelPolling is active via side effects
  void channelPolling;
