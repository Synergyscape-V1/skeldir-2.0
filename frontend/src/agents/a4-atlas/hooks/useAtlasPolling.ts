...
export function useAtlasPolling() {
  ...
  const cp = usePollingManager({ intervalMs: 300_000, onPoll: useCallback(async () => {}, []) });
  void cp;

  return { state, isPolling: dp.isActive, isUpdating, lastUpdated: dp.lastPollTime,
    retry: useCallback(() => { setState({ status: 'loading' }); setTimeout(() => setState(FIXTURES.ready), 800); }, []) };
}
