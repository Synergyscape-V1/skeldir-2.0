...
import { BudgetScenarioDetailV2 } from "../budget/components/BudgetScenarioDetailV2";
...
export function AgentShellBudgetScenarioDetail({
  theme,
  density = 100,
}: {
  theme: AgentTheme;
  ...
  return (
    <div
      className="agent-shell-root budget-optimizer-shell"
      style={
        {
          "--theme-bg": theme.bg,
      ...
      >
        ...
        <div className="nav-items-wrap">
          <nav className="nav-items" aria-label="Primary">
            <ShellNavItems activeRoute="Budget" activeChild="budget-scenarios" textColor={theme.text} />
          </nav>
        </div>
      </header>
      <main
        className="canvas budget-optimizer-canvas"
        style={{
          overflow: "auto",
          minHeight: "auto",
          // Match Command Center / Channel Comparison: strip aligns to canvas top-left (no default .canvas inset).
          margin: 0,
          padding: 0,
          paddingBottom: 0,
        }}
      >
        <BudgetScenarioDetailV2 />
      </main>
    </div>
  );
}
