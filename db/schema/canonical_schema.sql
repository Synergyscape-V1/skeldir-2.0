...
CREATE TABLE public.bayesian_artifacts (
    ...
    CONSTRAINT ck_bayesian_artifacts_internal_uri CHECK ((((lifecycle_status)::text = ANY ((ARRAY['pruned'::character varying, 'rejected'::character varying])::text[])) OR (((artifact_uri_internal)::text = (artifact_ref)::text) AND ((artifact_uri_internal)::text ~ '^b24://artifact/[a-f0-9-]{36}/[a-z0-9_]{3,32}/[a-f0-9]{12}$'::text)))),
