...
B2.5-P13 Correctives XV-XVI, topology and reproduction authority.
...
* **PostgreSQL 16 or later as evidence for the P13 claim.** Every P13 migration,
  RLS, Trust, and production-topology proof is pinned to PostgreSQL 15. Other
  repository workflows do use PostgreSQL 16 for their own surfaces; they do not
  establish P13 cross-version parity, and this matrix makes no such claim.
...
| **Local trust suite (bare database)** | `REPRODUCIBLE_GREENFIELD.md` §3 manual path | `postgres` only, with the seven least-privilege roles | Full `backend/tests/trust` suite, migration/role/RLS checks, C13/C14/C15/C16 closure validators |
...
| **Independent production topology proof** | `B2.5-P13 INDEPENDENT PRODUCTION TOPOLOGY REPRODUCTION.md` | Builds the unmodified production Bayesian image; provisions PostgreSQL 15; starts distinct execution and dispatch-publisher workers plus beat | Repeatable four-chain sampling, diagnostics/profile agreement, distinct-principal task flow, persisted posterior evidence, and positive Trust composition |
...
defect. Use the independent production-topology runbook when the claim under
test is the shipped image and distinct worker principals, or the forensic E2E
surface for the broader worker-physics gates.

## Issuance-consequence database custody

B2.5-P13 Corrective XVI added a seventh least-privilege login principal,
`app_trust_issuer`. It exists because recording a completed issuance is the act
of asserting that a private key physically produced a signature, and a CHECK
constraint can only prove the *shape* of that assertion. Any principal able to
write the audit row can also write correctly shaped bytes, so shape alone left
the ordinary `app_user` credential able to manufacture completed-issuance
history.

| Property | Value |
|---|---|
| Role | `app_trust_issuer`, `NOSUPERUSER`, `NOBYPASSRLS`, not a member of `app_rw`, `app_ro`, `app_user`, or `app_worker` |
| Table privileges | `SELECT, UPDATE` on `public.trust_access_log` only |
| Enforced by | `trust_access_log_issuance_authority_guard`, a `BEFORE INSERT OR UPDATE` trigger keying on `session_user`, which no `SET ROLE` can forge |
| Provisioned by | `scripts/database/prepare_migration_authority_boundary.py` (`--trust-issuer-user` / `--trust-issuer-password`) |
| Consumed by | `TRUST_ISSUANCE_DATABASE_URL`, read only by `backend/app/trust/issuance_session.py` |
| CI wiring | `.github/actions/setup-postgres-ci` exports the DSN to `$GITHUB_ENV`, so every job that provisions the boundary receives it |

If `TRUST_ISSUANCE_DATABASE_URL` is unset, the application falls back to the
ordinary runtime DSN. Against a provisioned schema the trigger then refuses
every consequence-bearing transition, so a misconfigured deployment fails closed
...
| PostgreSQL 15 + seven least-privilege roles + `alembic upgrade head` | `PASS-INFRASTRUCTURE` | 2026-08-30, Corrective XVI |
| Ordinary runtime principal physically refused every completed-issuance claim (15-case role x state matrix) | `PASS-SEMANTIC-FALSIFICATION` | 2026-08-30, Corrective XVI |
...
| C16 bidirectional issuance truth (6 PostgreSQL falsifiers) | `PASS-SEMANTIC-FALSIFICATION` | 2026-08-30, Corrective XVI |
| Production image, four-chain sampling twice, distinct publisher/worker principals, positive confidence | `PASS-INFRASTRUCTURE` + `PASS-SEMANTIC-FALSIFICATION` | 2026-08-30, `IEC-XVI-02` |
