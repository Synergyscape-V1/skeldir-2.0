...
Each record includes `route`, `cluster`, `primary_query`, `secondary_queries`, `proof_authority_routes`, `content_status`, `indexable`, `sitemap_required`, `schema_type` (`CollectionPage` for the hub, `WebPage` for evidence detail), `owner`, `last_reviewed`, and `similarity_group` for anti-spam grouping.
