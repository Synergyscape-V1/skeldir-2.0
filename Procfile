...
# process environment (foreman-style managers share one environment,
# so the blanking must be explicit per line, exactly like the C7
# DATABASE_URL override below), the worker fails closed at startup if
# the credential is present (assert_worker_ingress_isolation), and the
# database denies ingress authorship to every non-ingress principal.
# This line deliberately keeps the API DSN:
...
worker_bayesian: cd backend && SKELDIR_CELERY_WORKER_ROLE=bayesian DATABASE_URL=$WORKER_DATABASE_URL B26_P2_INGRESS_DATABASE_URL= SKELDIR_CELERY_INCLUDE_BAYESIAN_TASKS=1 celery -A app.celery_app.celery_app worker --loglevel=info --queues=bayesian
...
worker_bayesian_publisher: cd backend && SKELDIR_CELERY_WORKER_ROLE=bayesian_publisher DATABASE_URL=$PUBLISHER_DATABASE_URL B24_DISPATCH_PUBLISHER_DATABASE_URL=$PUBLISHER_DATABASE_URL B26_P2_INGRESS_DATABASE_URL= celery -A app.celery_app.celery_app worker --loglevel=info --queues=bayesian_publisher --concurrency=1
...
worker_b23: cd backend && DATABASE_URL=$B23_WORKER_DATABASE_URL B23_WORKER_DATABASE_URL=$B23_WORKER_DATABASE_URL B26_P2_INGRESS_DATABASE_URL= SKELDIR_B23_REQUIRE_WORKER_DSN=1 celery -A app.celery_app.celery_app worker --loglevel=info --queues=b23_match_engine --concurrency=${B23_WORKER_CONCURRENCY:-2} --prefetch-multiplier=1
...
relay_b26_p2: cd backend && DATABASE_URL=$B26_P2_RELAY_DATABASE_URL B26_P2_INGRESS_DATABASE_URL= SKELDIR_CELERY_WORKER_ROLE=b26_p2_relay celery -A app.celery_app.celery_app worker --loglevel=info --queues=b26_p2_relay --concurrency=1 --prefetch-multiplier=1
...
beat: cd backend && DATABASE_URL=$B26_P2_BEAT_DATABASE_URL B26_P2_INGRESS_DATABASE_URL= celery -A app.celery_app.celery_app beat --loglevel=info
